...
export const DevToolToggle: React.FC = () => {
  ...
  const [roleOverride, setRoleOverride] = useState<"" | "super_admin" | "owner" | "admin" | "doctor" | "staff" | "receptionist">(() => {
      ...
      {open ? (
        <div className="bg-card border shadow-lg rounded-md p-4 space-y-2 w-56">
          ...
          <div>
            ...
            <div className="flex gap-1 flex-wrap">
              {["super_admin", "owner", "admin", "doctor", "staff", "receptionist"].map((role) => (
                <Button
                  key={role}
                  size="sm"
